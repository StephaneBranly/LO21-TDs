...
ExpressionManager::Handler ExpressionManager::hand = ExpressionManager::Handler();
ExpressionManager &ExpressionManager::getInstance()
{
    if (hand.instance == nullptr)
        hand.instance = new ExpressionManager; // seul objet cr��
...
void ExpressionManager::removeExpression(Expression &e)
{
    ...
    delete exps[i]; // on d�salloue l'objet expression point�
    nb--;
...
{
    for (size_t i = 0; i < nb; i++)
        delete exps[i]; // d�sallocation d'un objet
    delete[] exps;      // d�sallocation d'un tableau de pointeurs
};
...
/* ExpressionManager& ExpressionManager::operator=(const ExpressionManager& m)
    {
        if (this != &m) // manager = manager ne s'auto d�truira pas
        {
...
void Pile::agrandissementCapacite()
{
    ...
    items = newtab;
    nbMax = (nbMax + 1) * 2;
    delete[] old; // il vaut mieux faire le delete en dernier sur une copie de pointeur afin d'�viter une exception
};
...
void Controleur::commande(const std::string &c)
{
    ...
    else // cas ou c'est un p-e op�rateur
    {
        if (c == "+" || c == "-" || c == "*" || c == "/") // cas ou on a un op�rateur
        {
            if (expAff.taille() >= 2)
            {
